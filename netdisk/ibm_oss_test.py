...
def create_bucket(bucket_name):
    ...
    try:
        cos_cli.create_bucket(Bucket=bucket_name,
...
def create_text_file(bucket_name, item_name, file_text):
    print("Creating new item: {0} in bucket: {1}".format(
        item_name, bucket_name))
    try:
        cos_cli.put_object(Bucket=bucket_name, Key=item_name, Body=file_text)
...
def delete_item(bucket_name, item_name):
    ...
    try:
        cos_cli.delete_object(Bucket=bucket_name, Key=item_name)
...
def upload_large_file(bucket_name, item_name, file_path):
    ...
    transfer_mgr = ibm_boto3.s3.transfer.TransferManager(
        cos_cli, config=transfer_config)
...
def main():
    try:
        ...
        create_text_file(new_bucket_name, new_text_file_name,
                         new_text_file_contents)
        ...
        upload_large_file(new_bucket_name, new_large_file_name,
                          new_large_file_name)
